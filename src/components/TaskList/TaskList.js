...
class TaskList extends Component {
  ...
  render() {
    ...
    return <ul className="todo-list">{tabAll ? itemRender(items) : tabActive ? itemRender(items.filter((el) => el.status === true)) : tabCompleted ? itemRender(items.filter((el) => el.status === false)) : itemRender(items)}</ul>;
  }
}
