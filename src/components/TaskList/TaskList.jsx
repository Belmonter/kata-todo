  ...
  render() {
    ...
    const itemRender = (items) => {
      const itemData = items.map(({ status, descr, create, edit, id, createDate }) => {
        return <Task statusItem={status} descr={descr} create={create} edit={edit} key={id} id={id} createDate={createDate} onDeleteItem={onDeleteItem} onComplete={onComplete} onEdit={onEdit} editItemInput={editItemInput} onSubmitEdit={onSubmitEdit} onEditChange={onEditChange} />
      })
      return itemData
    }
